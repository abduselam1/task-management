            ...
            <div class="bg-white overflow-hidden shadow-sm py-10 sm:rounded-lg">
                <div class="flex justify-between items-centers mx-5">
                    <div x-data="{showProject:false}" class="relative  inline-block text-left">
                        <div>
                            <button @click="showProject = !showProject" type="button" class="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50" >
                            Select a project
                        ...
                        <div x-show="showProject" class="absolute left-0 mt-2 w-full z-20 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            
                                <!-- Active: "bg-gray-100 text-gray-900", Not Active: "text-gray-700" -->
                                @foreach ($projects as $project)
                                    
                                    <a href="#" class="text-gray-700 block px-4 py-2 text-sm">{{$project->name}}</a>
                                @endforeach
                                
                            
                        </div>
                    </div>
                    <div x-data="{'showCreateProject':@entangle('showCreateProject').live}" class=" ">
                ...
                @if (session()->has('project'))
                    <div class="px-6 py-3 w-full rounded-sm mt-3 mx-2 bg-green-500/50">
